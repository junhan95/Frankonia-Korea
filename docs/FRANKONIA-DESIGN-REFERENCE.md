...
> **우선순위 안내 (2026-08-07).** 렌더링 포맷의 최종 기준은 저장소 루트의 **`디자인.md`** 이며, 그 원천은 CyberShield 제품 사이트(www.frankonia-cybershield.com)의 실측값이다. 그 사이트를 포팅해 두었던 `app/cybershield/`는 2026-08-11 요약본 전환으로 삭제했으므로, 재실측이 필요하면 제품 사이트를 직접 본다. 이 문서는 토큰 사전과 브랜드 배경 자료로 계속 유효하지만, 두 문서가 어긋나는 항목은 `디자인.md`가 이긴다.
>
