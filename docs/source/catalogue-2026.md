...
## 6. 본문 이관 완료 — 개요·산업군 4개·챔버 형식 6개 (2026-08-10)

"콘텐츠 준비 중" 밴드가 남아 있던 11개 페이지를 카탈로그로 채웠다. **챔버 브랜치에서
비어 있는 페이지는 이제 Downloads 하나뿐이다.**

| 페이지 | 카탈로그 인쇄 p. | 리드 | 표 | 체크리스트 |
|---|---|---|---|---|
| `/chambers` | 3 · 5 · 66 | 2문단 | — | 2그룹 20항목 + 마무리 |
| `/chambers/industry/automotive` | 40–45 | 3문단 | 3개 13행 | 2그룹 13항목 |
| `/chambers/industry/military` | 54–55 | 3문단 | 2개 8행 | 2그룹 8항목 |
| `/chambers/industry/commercial` | 10–37 종합 | 2문단 | 1개 9행 | 2그룹 10항목 |
| `/chambers/industry/powertrain` | 48–49 | 3문단 | 3개 11행 | 2그룹 11항목 |
| `/chambers/type/sac` | 18–37 | 3문단 | 4개 20행 | 2그룹 10항목 |
| `/chambers/type/fac` | 16–17 · 26–27 | 3문단 | 1개 3행 | 2그룹 12항목 |
| `/chambers/type/chc` | 12–13 · 54–55 | 3문단 | 1개 6행 | 2그룹 8항목 |
| `/chambers/type/component` | 12 · 40–41 · 48–49 | 3문단 | 1개 7행 | 2그룹 10항목 |
| `/chambers/type/rvc` | 50–51 | 3문단 | 3개 12행 | 1그룹 7항목 |
| `/chambers/type/shielded-room` | 10–11 | 3문단 | 1개 8행 | 2그룹 14항목 |

### 표를 넣은 이유 — 모델 목록이 담지 못하는 것

`chamberModels`는 제품 하나에 항목 하나다. 본사가 제품으로 공개하는 단위가 그것이기
때문이다. 그런데 카탈로그는 SAC-3 Plus를 S·M·L·기본 네 크기로, SAC-10V를 일곱 구성으로,
컴팩트 챔버를 각각 두 개씩 찍어 준다. **시험실이 실제로 고르는 것은 이 크기들이다.**

앞서 3절에서 "반영한다면 모델 목록이 아니라 모델 상세 페이지의 사양표에 들어가야 한다"고
적어 둔 항목들 — MIL CPC, MIL CHC / DO-160, MIL-STD Advanced의 Pyramid·Hybrid 구분 —
이 그 사양표에 들어갔다. 모델 목록에는 넣지 않았고, 표 아래 주석에 **"2026 카탈로그의
표기이며 본사 웹사이트 제품 목록에는 아직 없다"**고 명시했다. CHC L·CHC Plus L도 같다.

### 셀 안의 값은 번역하지 않는다

표의 제목·열 머리·주석만 번역한다. 치수, Quiet Zone 문구, 규격 번호, 스터러 사양은
카탈로그 표기 그대로다 — `spec`에 적용한 규칙과 같고, 이유도 같다. 독자가 도면·견적서와
맞춰 볼 값이다. 단위 표기만 3절의 결정대로 `m` → `mm`로 고쳐 옮겼다.

### 새로 만든 것

- `SpecTable` 타입과 `.spec-table` — 이 사이트의 hairline 문법(잉크 상단선, 행마다 헤어라인,
  hover 시 행 반전, tabular numerals) 위에 올린 표. 셀은 `\n`으로 쪼개져 첫 줄이 수치,
  나머지가 카탈로그가 그 아래 작게 인쇄하는 한정구가 된다
- `Plate` 타입 — 기존 `figure`에 `alt`·`caption`을 더했다. 장식용 띠였던 기존 도판은
  `alt=""`를 유지하고, 논거가 되는 새 도판만 둘 다 갖는다
- `figureRow` — 개요 페이지의 3분할 도판. 기존 `.figure-row` CSS를 그대로 쓴다
- 밴드 교대(`.alt`)를 하드코딩에서 계산으로 바꿨다. 페이지마다 존재하는 밴드가 다르므로
  (인덱스는 모델 목록이 있고 파노라마가 없다, References는 그 반대다) 위에 이웃이 있는지를
  선언 지점에서는 알 수 없다

### 카탈로그 오타

3절에 적은 단위 오기(`m` → `mm`) 외에 이번에 확인한 것:

- SAC-10-3/P와 SAC-10-4/P의 치수가 `21,680 × 13,730 × 8,550`으로 **동일하다**. 추출
  오류를 의심해 인쇄 p.37을 직접 렌더해 확인했다 — 카탈로그가 실제로 그렇게 인쇄한다.
  Quiet Zone만 ø3.0 m와 ø4.0 m로 다르다. 그대로 옮겼다
- 철자 오류 — p.13 `hieght`, p.35 `Optmized`, p.37 `Noteably`, p.49 `patneted`,
  p.54 `Compact Parymid Chamber`. 본문으로 옮긴 문장에 걸린 것은 정상 표기로 고쳤다
  (앞서 Automation 스프레드의 `compliant wiht`를 고친 것과 같다)
- 추출 아티팩트이지 오타가 아닌 것 — p.54의 `MIL CMHCIL&-STMDIL CPC`는 겹쳐 놓인 두
  캡션(`MIL CHC & MIL-STD`, `MIL CPC`)을 `pdftotext`가 한 줄로 엮은 결과다. 인쇄면을
  직접 렌더해 확인했다
