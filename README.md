...
- **랜딩에 이미지가 로고 한 장뿐이다.** 대형 무향 챔버를 파는 회사의 첫 화면이
  텍스트 벽이다. 저장소에 **챔버 사진·렌더가 한 장도 없는 것**이 실질적 병목이고
  (Company 7장, CyberShield 13장 + 영상은 있다), 기획서도 히어로 배경을 "추후 본사
  챔버 렌더/영상으로 교체"로 미뤄 두었다. 본사 자료 확보가 선행되어야 한다.
