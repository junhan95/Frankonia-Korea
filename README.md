...
- **히어로 아이브로우가 WCAG AA에 미달한다.** `.hero .tag`는 15px·800 굵기의
  `--primary`이고 잉크 위에서 **3.08:1**이다. 15px는 "큰 텍스트"가 아니라 4.5:1이
  필요하다. 이미지와 무관한 기존 문제이고(레드/잉크 색 쌍 자체의 값), 같은 레드
  아이브로우라도 밝은 배경 위에서는 4.81:1로 통과한다. 잉크 밴드 위의 것만
  해당된다 — 크기를 18.66px 이상으로 올리거나(그러면 큰 텍스트 기준 3:1 적용)
  색을 바꾸는 디자인 결정이 필요하다.
